...
<body class="bg-gray-100 font-sans antialiased flex h-screen overflow-hidden">
    ...
    <aside class="w-64 bg-gray-900 text-white flex flex-col">
        ...
        <nav class="flex-1 px-4 py-6 space-y-2 overflow-y-auto custom-scrollbar">
            ...
            <p class="px-4 text-xs font-bold text-gray-500 uppercase tracking-wider mb-2 mt-2">Analisis & Laporan</p>
            ...
            <p class="px-4 text-xs font-bold text-gray-500 uppercase tracking-wider mb-2 mt-6">Operasional Bengkel</p>
            ...
            <a href="{{ route('admin.vehicles.index') }}" class="flex items-center px-4 py-3 rounded-xl transition {{ request()->routeIs('admin.vehicles.*') ? 'bg-red-600 text-white shadow-lg shadow-red-500/30' : 'text-gray-300 hover:bg-gray-800 hover:text-white' }}">
                <i class="fas fa-motorcycle w-6"></i> Kendaraan Pelanggan
            </a>
            
            <p class="px-4 text-xs font-bold text-gray-500 uppercase tracking-wider mb-2 mt-6">Inventaris & Gudang</p>
            ...
            <a href="{{ route('admin.spareparts.index') }}" class="flex items-center px-4 py-3 rounded-xl transition {{ request()->routeIs('admin.spareparts.*') ? 'bg-red-600 text-white shadow-lg shadow-red-500/30' : 'text-gray-300 hover:bg-gray-800 hover:text-white' }}">
                <i class="fas fa-cogs w-6"></i> Master Suku Cadang
            </a>
            
            <a href="{{ route('admin.purchases.index') }}" class="flex items-center px-4 py-3 rounded-xl transition {{ request()->routeIs('admin.purchases.*') ? 'bg-red-600 text-white shadow-lg shadow-red-500/30' : 'text-gray-300 hover:bg-gray-800 hover:text-white' }}">
                <i class="fas fa-box-open w-6"></i> Pembelian & Restok
            </a>

            <p class="px-4 text-xs font-bold text-gray-500 uppercase tracking-wider mb-2 mt-6">Karyawan</p>
            <a href="{{ route('admin.mechanics.index') }}" class="flex items-center px-4 py-3 rounded-xl transition {{ request()->routeIs('admin.mechanics.*') ? 'bg-red-600 text-white shadow-lg shadow-red-500/30' : 'text-gray-300 hover:bg-gray-800 hover:text-white' }}">
                <i class="fas fa-wrench w-6"></i> Data Mekanik
            </a>
    ...
    <main class="flex-1 flex flex-col h-screen overflow-y-auto bg-gray-50">
        ...
        <header class="bg-white shadow-[0_2px_15px_rgb(0,0,0,0.03)] border-b border-gray-100 px-8 py-5 flex justify-between items-center sticky top-0 z-20">
            ...
            <div class="flex items-center gap-3 bg-gray-50 px-4 py-2 rounded-2xl border border-gray-100">
                <div class="w-10 h-10 bg-red-100 text-red-600 rounded-xl flex items-center justify-center font-black shadow-inner">
                    {{ substr(Auth::user()->name, 0, 1) }}
                </div>
                <div>
                    <p class="text-sm font-bold text-gray-800 leading-tight">{{ Auth::user()->name }}</p>
                    <p class="text-xs font-semibold text-gray-400 uppercase tracking-wider">{{ Auth::user()->role }}</p>
                </div>
            </div>
        </header>
